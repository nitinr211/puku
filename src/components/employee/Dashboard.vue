      ...
      <v-content>
        <v-card>
          ...
          >
            <v-layout row wrap>
              <v-flex xs12 sm6 md3>
                 <router-link :to="{ name: 'EmployeeLeadsType', params: {type:'Contacted'}}">
              ...
              <v-flex xs12 sm6 md3>
                 <router-link :to="{ name: 'EmployeeLeadsType', params: {type:'Qualified'}}">
              ...
              <v-flex xs12 sm6 md3>
                 <router-link :to="{ name: 'EmployeeLeadsType', params: {type:'Working'}}">
              ...
              <v-flex xs12 sm6 md3>
                <router-link :to="{ name: 'EmployeeLeadsType', params: {type:'Proposal Sent'}}">
        ...
        >
          <template slot="items" slot-scope="props"> 
            <td class="text-xs-left" v-if="props.item.leadRecordUpdateForEmployee == true" @click="updateLeadRecords(props.item.id)">
            ...
            <td class="text-xs-left">{{ props.item.edited_at }}</td> 
...
export default {
      ...
      updateLeadRecords(id){
        var db = firebase.firestore(); 
         let leadRecordsUpdateRef = db.collection('leads').doc(id);
              ...
              leadRecordUpdateForEmployee:false          
        })
      }
  ...
  created(){
      ...
      // Show All Leads
      let ref = db.collection('leads').orderBy('timestamp', 'desc').where("employeeId", "==", this.employeeId).where("employeeSelect", "==", "Yes")
        ...
        snapshot.docChanges().forEach(change => {
          if(change.type == 'added'){
            ...
            this.leads.push({            
              ...
              leadRecordUpdateForEmployee:doc.data().leadRecordUpdateForEmployee,
      ...
      db.collection('leads').where("status.text", "==", "Contacted").where("employeeId", "==", this.employeeId)
     .get()
     .then(snapshot => {
      ...
      db.collection('leads').where("status.text", "==", "Qualified").where("employeeId", "==", this.employeeId)
     .get()
     .then(snapshot => {
      ...
      // Total Working Leads
      db.collection('leads').where("status.text", "==", "Working").where("employeeId", "==", this.employeeId)
     .get()
     .then(snapshot => {
      ...
      db.collection('leads').where("status.text", "==", "Proposal Sent").where("employeeId", "==", this.employeeId)
     .get()
     .then(snapshot => {
